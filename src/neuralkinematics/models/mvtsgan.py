...
class MVTSGAN(object):
    ...
    def __init__(self, input_streams, output_streams = None, lr = 0.001, l2_gen = 0.0001, saturation = True, epochs = 100, batch_len = 100, rand_noise = False, verbose = False, lr_reduce=True):
        ...
        self.fine_optimizer = tf.keras.optimizers.Adam(lr = lr *0.05)
        self.l2_gen = l2_gen
    ...
    def make_generator(self):
        ...
        self.gen_conv.add(tf.keras.layers.Conv1D(100, 3, padding='same', kernel_regularizer=tf.keras.regularizers.l1_l2(l2=self.l2_gen), input_shape=(None, self.input_streams)))
        self.gen_conv.add(tf.keras.layers.Conv1D(100, 3, padding='same', activation='tanh'))
        self.gen_conv.add(tf.keras.layers.Conv1D(100, 2, padding='same'))
        self.generator.add(self.gen_conv)
        #
        ...
        self.gen_dense.add(tf.keras.layers.Dense(100))
    ...
    def train_step(self, real_input, real_target):
        ...
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            ...
            rec_loss = tf.reduce_mean(tf.math.square(reconstructed - tf.cast(real_target, tf.float32)))
            # rec_reg = 1 - 1. * i / self.n_epochs
            generator_loss += 0.0* rec_loss
    ...
    @tf.function
    def fine_tune(self, real_input, real_target):
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
        ...
        self.fine_optimizer.apply_gradients(zip(gen_gradient, self.generator.trainable_variables))
        self.fine_optimizer.apply_gradients(zip(dis_gradient, self.discriminator.trainable_variables))

        return generator_loss, discriminator_loss

    def fit(self, input_signal, target_signal, fine_tune = False):
        self.make_generator()
        ...
        for i in range(self.n_epochs):
            ...
            for b in range(n_batches):
                ...
                if fine_tune == True:
                    g, d = self.fine_tune(input_signal[b_st:b_en].reshape(1, b_en - b_st, self.input_streams),
                                      target_signal[b_st:b_en].reshape(1, b_en - b_st, self.output_streams))
                else:
