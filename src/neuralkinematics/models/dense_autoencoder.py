...
class DenseAutoencoder(object):
    ...
    def make_encoder(self):
        ...
        self.encoder.add(tf.keras.layers.Dense(100, input_shape = [None, self.streams]))
        self.encoder.add(tf.keras.layers.Dense(50))
    ...
    def make_decoder(self):
        ...
        self.decoder.add(tf.keras.layers.Dense(100))
    ...
    def fit(self, input_signal):
        ...
        for i in range(self.n_epochs):
            ...
            for b in range(n_batches):
                ...
                l = self.train_step(input_signal[b_st:b_en])
                if np.isnan(l).any() :
                    return losses
                loss += np.mean(l)
